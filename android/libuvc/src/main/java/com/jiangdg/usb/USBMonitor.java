...
public final class USBMonitor {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				...
				synchronized (USBMonitor.this) {
					...
					if (device != null) {
						...
						XLogWrapper.i(TAG,
								"onReceive: USB_PERMISSION for device=" + device.getDeviceName() + ", VendorId="
										+ device.getVendorId() + ", ProductId=" + device.getProductId() + ", granted="
										+ granted);
				...
				if (device != null) {
					...
					if (hasPermission(device)) {
						XLogWrapper.i(TAG,
								"onReceive: Already have permission for attached device, processing connection");
						processConnect(device);
					} else {
	...
	private USBMonitor(final Context context) {
		...
		mWeakContext = new WeakReference<Context>(context.getApplicationContext()); // Use application context to
																					// prevent leaks
	...
	public void destroy() {
		...
		// Clear the singleton instance if this is the singleton instance being
		// destroyed
		if (sInstance == this) {
	...
	public final boolean hasPermission(final UsbDevice device) throws IllegalStateException {
			...
			synchronized (mHasPermissions) {
				...
				if (cachedDevice != null && cachedDevice.get() != null) {
					...
					if (systemHasPermission) {
						...
						if (DEBUG)
							XLogWrapper.i(TAG,
									"hasPermission: permission revoked for device: " + device.getDeviceName());
						mHasPermissions.remove(deviceKey);
						return false;
					}
				}
		...
		if (DEBUG)
			XLogWrapper.i(TAG, "hasPermission checking for device: " + device.getDeviceName() +
			...
			synchronized (mHasPermissions) {
				...
				if (hasPermission) {
					if (!wasInCache) {
						if (DEBUG)
							XLogWrapper.i(TAG,
									"updatePermission: adding device to permission cache: " + device.getDeviceName());
					...
					if (wasInCache) {
						if (DEBUG)
							XLogWrapper.i(TAG, "updatePermission: removing device from permission cache: "
	 ...
	 * @return true if request process successfully started or already have
	 *         permission
	 */
	public synchronized boolean requestPermission(final UsbDevice device) {
		...
		// Check if we already have permission first
		...
		// Only for Android 9+ and IF we don't have permission, we might want to clear
		// local cache
		// but let's NOT call resetDevicePermission which might be too aggressive
		if (Build.VERSION.SDK_INT >= 28) {
			XLogWrapper.i(TAG, "Android 9+ detected, clearing internal cache before request");
			synchronized (mHasPermissions) {
				mHasPermissions.remove(getDeviceKey(device, true));
			}
		}
		...
		public void run() {
			...
			synchronized (mHasPermissions) {
				...
				if (n != mDeviceCounts) {
					...
					if (DEBUG)
						XLogWrapper.i(TAG,
								"Device count changed from " + mDeviceCounts + " to " + n + ", checking permissions");
				}
	...
	private void processCancel(final UsbDevice device) {
		...
		if (context != null) {
			...
			if (isFirstRun) {
				XLogWrapper.w(TAG,
						"processCancel: This appears to be a first run, permission dialog may not have been shown");
				prefs.edit().putBoolean("first_run", false).apply();
		...
		XLogWrapper.i(TAG,
				"processCancel: Permission intent status: " + (mPermissionIntent != null ? "valid" : "NULL"));
	...
	public static final class UsbControlBlock implements Cloneable {
		...
		private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			if (DEBUG)
				XLogWrapper.i(TAG, "UsbControlBlock:constructor");
			...
			if (DEBUG) {
				if (mConnection != null) {
					...
					XLogWrapper.i(TAG, String.format(Locale.US, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name,
							desc, busnum, devnum));
				} else {
			...
			if (mConnection == null) {
				...
				if (mConnection == null) {
					throw new IllegalStateException(
							"openDevice failed. device may already be removed or have no permission, dev = " + device);
				}
			}
			mInfo = updateDeviceInfo(monitor.mUsbManager, device, null);
			mWeakMonitor = new WeakReference<USBMonitor>(monitor);
