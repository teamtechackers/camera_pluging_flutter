...
public class UVCCamera {
	...
	// these fields from here are accessed from native code and do not change name
	// and remove
	...
	// until here
	/**
	 * the sonctructor of this class should be call within the thread that has a
	 * looper
	...
	public synchronized void open(final UsbControlBlock ctrlBlock) {
		...
		try {
			...
			// long id_camera, int venderId, int productId, int fileDescriptor, int busNum,
			// int devAddr, String usbfs
		...
		if (result != 0) {
			throw new UnsupportedOperationException("open failed:result=" + result + "----->" +
					...
					+ ";productId=" + (mCtrlBlock == null ? "" : mCtrlBlock.getProductId()) + ";fileDescriptor="
					+ (mCtrlBlock == null ? "" : mCtrlBlock.getFileDescriptor())
					+ ";busNum=" + (mCtrlBlock == null ? "" : mCtrlBlock.getBusNum()) + ";devAddr="
					+ (mCtrlBlock == null ? "" : mCtrlBlock.getDevNum())
	...
	public synchronized void close() {
		...
		if (DEBUG)
			XLogWrapper.v(TAG, "close:finished");
	}
	...
	public void setPreviewSize(final int width, final int height) {
		setPreviewSize(width, height, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, mCurrentFrameFormat,
	...
	public void setPreviewSize(final int width, final int height, final int min_fps, final int max_fps,
		...
		if (mNativePtr != 0) {
			final int result = nativeSetPreviewSize(mNativePtr, width, height, min_fps, max_fps, frameFormat,
					bandwidthFactor);
			if (result != 0)
		...
		if (mNativePtr != 0) {
			if ((mControlSupports == 0) || (mProcSupports == 0)) {
				...
				if (false) {
					...
					XLogWrapper.v(TAG, String.format("Brightness:min=%d,max=%d,def=%d", mBrightnessMin, mBrightnessMax,
							mBrightnessDef));
					XLogWrapper.v(TAG,
							String.format("Contrast:min=%d,max=%d,def=%d", mContrastMin, mContrastMax, mContrastDef));
					XLogWrapper.v(TAG, String.format("Sharpness:min=%d,max=%d,def=%d", mSharpnessMin, mSharpnessMax,
							mSharpnessDef));
					...
					XLogWrapper.v(TAG, String.format("Saturation:min=%d,max=%d,def=%d", mSaturationMin, mSaturationMax,
							mSaturationDef));
					...
					XLogWrapper.v(TAG, String.format("WhiteBlance:min=%d,max=%d,def=%d", mWhiteBlanceMin,
	...
	private final native int nativeConnect(long id_camera, int venderId, int productId, int fileDescriptor, int busNum,
			int devAddr, String usbfs);
	...
	private static final native int nativeSetPreviewSize(final long id_camera, final int width, final int height,
