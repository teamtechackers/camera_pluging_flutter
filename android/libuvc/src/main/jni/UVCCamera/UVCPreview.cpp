 ...
 * Files in the jni/libjpeg, jni/libusb, jin/libuvc, jni/rapidjson folder may
 * have a different license, see the respective files.
 */
...
UVCPreview::UVCPreview(uvc_device_handle_t *devh)
    : mPreviewWindow(NULL), mCaptureWindow(NULL), mDeviceHandle(devh),
      requestWidth(DEFAULT_PREVIEW_WIDTH),
      requestHeight(DEFAULT_PREVIEW_HEIGHT),
      requestMinFps(DEFAULT_PREVIEW_FPS_MIN),
      requestMaxFps(DEFAULT_PREVIEW_FPS_MAX), requestMode(DEFAULT_PREVIEW_MODE),
      requestBandwidth(DEFAULT_BANDWIDTH), frameWidth(DEFAULT_PREVIEW_WIDTH),
      frameHeight(DEFAULT_PREVIEW_HEIGHT),
      frameBytes(DEFAULT_PREVIEW_WIDTH * DEFAULT_PREVIEW_HEIGHT * 2), // YUYV
      frameMode(0), previewBytes(DEFAULT_PREVIEW_WIDTH *
                                 DEFAULT_PREVIEW_HEIGHT * PREVIEW_PIXEL_BYTES),
      previewFormat(WINDOW_FORMAT_RGBA_8888), mIsRunning(false),
      mIsCapturing(false), captureQueu(NULL), mFrameCallbackObj(NULL),
      mFrameCallbackFunc(NULL), callbackPixelBytes(2) {
  ...
  pthread_mutex_lock(&preview_mutex);
  {
    if (mPreviewWindow != preview_window) {
      ...
      if (LIKELY(mPreviewWindow)) {
        ANativeWindow_setBuffersGeometry(mPreviewWindow, frameWidth,
                                         frameHeight, previewFormat);
      }
    }
  }
  ...
  pthread_mutex_lock(&capture_mutex);
  {
    if (isRunning() && isCapturing()) {
      ...
      if (mFrameCallbackObj) {
        ...
        pthread_cond_wait(&capture_sync,
                          &capture_mutex); // wait finishing capturing
      }
    }
...
int UVCPreview::startPreview() {
  ...
  if (!isRunning()) {
    ...
    if (UNLIKELY(result != EXIT_SUCCESS)) {
      LOGW("UVCCamera::window does not exist/already running/could not create "
           "thread etc.");
      mIsRunning = false;
      pthread_mutex_lock(&preview_mutex);
      {
...
int UVCPreview::stopPreview() {
  ...
  if (LIKELY(b)) {
    ...
    if (mHasCapturing) {
      pthread_cond_signal(&capture_sync);
      if (capture_thread &&
          pthread_join(capture_thread, NULL) != EXIT_SUCCESS) {
...
//
//**********************************************************************
void UVCPreview::uvc_preview_frame_callback(uvc_frame_t *frame,
                                            void *vptr_args) {
  UVCPreview *preview = reinterpret_cast<UVCPreview *>(vptr_args);
  if UNLIKELY (!preview->isRunning() || !frame || !frame->frame_format ||
               !frame->data || !frame->data_bytes)
    return;
  // Only drop frames with wrong dimensions, NOT partial data frames
  if (UNLIKELY((frame->width != preview->frameWidth) ||
               (frame->height != preview->frameHeight))) {
    LOGI("dropped frame: wrong size (%d,%d)/(%d,%d)", frame->width,
         frame->height, preview->frameWidth, preview->frameHeight);
    return;
  }
  // For non-MJPEG (YUYV) frames with partial data, pad with zeros instead of
  // dropping
  if ((frame->frame_format != UVC_FRAME_FORMAT_MJPEG) &&
      (frame->actual_bytes < preview->frameBytes)) {
    // Zero-pad the missing bytes in the frame data
    if (frame->data_bytes >= preview->frameBytes) {
      memset((uint8_t *)frame->data + frame->actual_bytes, 0,
             preview->frameBytes - frame->actual_bytes);
      frame->actual_bytes = preview->frameBytes;
    }
  }
  ...
  if (LIKELY(!result)) {
    ...
    if (LIKELY(!result)) {
      ...
      if (LIKELY(mPreviewWindow)) {
        ANativeWindow_setBuffersGeometry(mPreviewWindow, frameWidth,
                                         frameHeight, previewFormat);
        int32_t wFmt = ANativeWindow_getFormat(mPreviewWindow);
        int32_t wW = ANativeWindow_getWidth(mPreviewWindow);
        int32_t wH = ANativeWindow_getHeight(mPreviewWindow);
        LOGI("ANativeWindow: req=(%d,%d,fmt=%d) actual=(%d,%d,fmt=%d)",
             frameWidth, frameHeight, previewFormat, wW, wH, wFmt);
      }
      pthread_mutex_unlock(&preview_mutex);
    } else {
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
  ...
  if (LIKELY(!result)) {
    clearPreviewFrame();
    if (pthread_create(&capture_thread, NULL, capture_thread_func,
                       (void *)this) == 0) {
      mHasCapturing = true;
    }
...
static void copyFrame(const uint8_t *src, uint8_t *dest, const int width,
                      int height, const int stride_src, const int stride_dest) {
...
int copyToSurface(uvc_frame_t *frame, ANativeWindow **window) {
  ...
  if (LIKELY(*window)) {
    ...
    if (LIKELY(ANativeWindow_lock(*window, &buffer, NULL) == 0)) {
      ...
      // use lower height
      const int h =
          frame->height < buffer.height ? frame->height : buffer.height;
...
// changed to return original frame instead of returning converted frame even if
// convert_func is not null.
uvc_frame_t *UVCPreview::draw_preview_one(uvc_frame_t *frame,
...
uvc_frame_t *UVCPreview::waitCaptureFrame() {
  ...
  {
    if (!captureQueu) {
      //  这里有阻塞的情况，替换成 pthread_cond_timedwait
      //  方法，设置相对的超时时间为 1s
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
  ...
  if (LIKELY(frame)) {
    ...
    if (mFrameCallbackObj) {
      ...
      jobject buf =
          env->NewDirectByteBuffer(callback_frame->data, callbackPixelBytes);
      if (iframecallback_fields.onFrame) {
